...
strongSwanを使用し、モダンで安全なIKEv2方式のVPNサーバーをセットアップします。ユーザー名・パスワード認証（EAP-MSCHAPv2）を採用しており、iOS, macOS, Windowsなどの標準VPNクライアントと互換性があります。
...
1.  スクリプト内の `PUBLIC_KEY` 変数を編集します。ランダム生成ではなく固定したい場合は `VPN_USER` と `VPN_PASSWORD` も設定できます。
2.  スクリプトをサーバーにコピーします。
3.  `root` としてサーバーに接続し、スクリプトに実行権限を与え（`chmod +x`）、実行します。

#### 実行後

- 実行後、サーバーのIPアドレス、VPNのユーザー名、パスワードが出力されます。
- **【最重要】** 同時に**CA証明書**の内容も表示されます。VPNに接続したい全てのデバイスで、この証明書テキストをファイル（例: `ca.cer`）として保存し、インストールする必要があります。これはデバイスごとに１回だけ必要な作業です。
- iOS/iPadOSの場合、証明書インストール後、`設定 > 一般 > 情報 > 証明書信頼設定`を開き、インストールしたCA証明書をオンにする必要があります。
- SSH経由で `vpsuser` としてポート `2222` で接続できます。
...
**現在のステータス:** 現在、開発を凍結しています。このスクリプトは**動作しません**。

**推奨:** モダンで安全、かつ正常に動作するVPNサーバーを構築するために、代わりに `strongswan-ikev2.sh` スクリプトを使用してください。
